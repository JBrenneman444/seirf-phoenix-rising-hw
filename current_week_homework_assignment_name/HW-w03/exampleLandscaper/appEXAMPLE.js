...
const startGame = () => {
  ...
  // automatically show NEXT tool in store
  player = {
    money: 0,
    tool: tools.shift()
  };
  // alerts are annoying - let's use a simple div
        // and update the innerHTML as needed
...
const message = (str) => {
  ...
  <h6>Money per lawn mowed: <span>${player.tool.earnings}</span></h6>
...
const renderStore = () => {
  ...
  if (tools.length > 0) {
    ...
    `;
    store.innerHTML += (player.money >= tools[0].purchasePrice)
     ? `<p><button class='purchase'> Purchase </button></p>` : 
     `<p>You can't afford this yet</p>`;
...
//starts Game after page loads all content, also creates Event Listener tied to landscape function/button
